    ...
    (async () => {
      ...
      try {
        ...
        const linkedSet = new Set(
          (linkedMembers ?? []).map((row: any) => String(row.github_login).toLowerCase()),
        );
        ...
        if (!projectOrgId && project.owner) {
          ...
          try {
            ...
            console.error("Failed to infer org for team linked members:", e);
          }
        }
        ...
        const ensure = (login: string, avatar = "") => {
          ...
          if (!m) {
            m = { login, name: login, avatar, contrib: 0, add: 0, del: 0, role: "contributor" };
            byLogin.set(login, m);
          }
  ...
  return (
    <>
      ...
      {loading && members.length === 0 ? (
        <div className="glass rounded-xl p-6 text-sm text-muted-foreground">Loading team…</div>
      ) : null}
      ...
      <Section title="Collaborators" icon={Users2} accent="primary">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {collaborators.length === 0 ? (
            ...
            collaborators.map((m) => (
              <MemberCard key={m.login} m={m} linked={linkedLogins.has(m.login.toLowerCase())} />
            ))
          )}
        </div>
      </Section>
